        ...
        <div className="space-y-4 mb-4">
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <div className="space-y-3">
              {[
                { label: '❤️ Like Potential', value: detailed.signals.like_potential },
                { label: '💬 Reply Potential', value: detailed.signals.reply_potential },
                { label: '🔁 Repost Potential', value: detailed.signals.repost_potential },
                { label: '🔗 Click Potential', value: detailed.signals.click_potential },
                { label: '🎥 Video Potential', value: detailed.signals.video_potential },
              ].map(({ label, value }) => (
                <div key={label}>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <div className="space-y-3">
              {[
                ...
                { label: 'Call to Action', value: detailed.content_quality.call_to_action },
              ].map(({ label, value }) => (
                <div key={label}>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-gray-600">{label}</span>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            <h3 className="font-semibold text-gray-900 mb-4 text-sm uppercase tracking-wide">Algorithmic Fit</h3>
            <div className="space-y-3">
              {[
                { label: 'Candidate Isolation', value: detailed.algorithmic_fit.candidate_isolation },
                { label: 'Newsworthiness', value: detailed.algorithmic_fit.newsworthiness },
                { label: 'Safety', value: detailed.algorithmic_fit.safety },
              ].map(({ label, value }) => (
                <div key={label}>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-gray-600">{label}</span>
